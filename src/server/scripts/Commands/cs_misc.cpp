...
public:
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, ItemTemplate const* itemTemplate, Optional<int32> _count)
    {
        ...
        // CUSTOM: GMs de nivel 3 solo pueden añadir/quitar items sobre sí mismos.
        // Comparación por GUID para cubrir también targets OFFLINE (soporte upstream #26714).
        // La consola/SOAP no tiene sesión y queda exenta.
        if (Player* executor = handler->GetSession() ? handler->GetSession()->GetPlayer() : nullptr)
        {
            if (player->GetGUID() != executor->GetGUID() && !handler->HasPermission(rbac::RBAC_PERM_COMMAND_ADDITEM_ANY_TARGET))
            {
